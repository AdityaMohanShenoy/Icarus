...
def runner(w):
    def GeneratePrompt():
        with open("chest_assets/prompts.txt", 'r') as file:
            ...
            finalprompt="it knew that it would melt off icarus's wings"
            for i in prompts:
    ...
    pygame.mixer.init()
    pygame.mixer.music.load("sounds/menon game.mp3")
    wings=pygame.mixer.Sound("sounds/wings.mp3")
    pygame.mixer.Sound.set_volume(wings,1)
    haswon=pygame.mixer.Sound("sounds/opening.mp3")
    pygame.mixer.Sound.set_volume(haswon,0.4)
    replay=True
    cover=True
    vict=False
    while replay:
